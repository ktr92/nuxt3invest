...
const changeItemColor = (
  ...
  d3.select(`[data-category=${category}] [data-color]`).attr(
    "style",
    `background-color: ${colorAttr === "hover" ? colorHover : prevColor}`
  )
}
...
const changePieColor = (
  ...
  if (prevColor) {
    ...
    if (d) {
      ...
      const tooltipText = `${d.data.category}: ${numberFormat(
        d.value.toFixed(2)
      )} ₽`
...
const showTooltip = (category: string, tooltipText: string) => {
  ...
  tooltip.x = Number(
    d3.select(`path[data-id="${category}"]`).attr("data-centerX")
  )
  tooltip.y = Number(
    d3.select(`path[data-id="${category}"]`).attr("data-centerY")
  )
}
